/**********************************************************************************************************************
* Description: This is the implementation file for the BedRoom3 class. It is a child of the abstract Spaces class. It
...
std::string BedRoom3::getRoomName() {
	return currentSpace;
}

// displays the items in the room
void BedRoom3::getItems() {
...
int BedRoom3::getNumItems() {
	return items.size();
}

// dispays the rooms interaction, if any, returns an integer the number of choices for the user
int BedRoom3::getInteraction() {
	if (!interaction) {
		std::cout << "Toby is sleeping in bed, do you want to wake him up?\n";
		std::cout << "1. Yes\n2. No\n";
		return 2;
	} else {
...
int BedRoom3::getInteractionInt() {
	return interaction;
}
...
void BedRoom3::setMoves(Space* room) {
	roomVects[0] = nullptr;
	roomVects[1] = nullptr;
	roomVects[2] = nullptr;
	roomVects[3] = room;
...
void BedRoom3::setMoves(Space* room1, Space* room2) {
	// // intentionally left empty
}

void BedRoom3::setMoves(Space* room1, Space* room2, Space* room3, Space* room4, Space* room5) {
	// // intentionally left empty
}

// the effect of the interaction, if any
void BedRoom3::interact(int choice) {
	if (choice == 1) {
		std::cout << "You wake Toby up and hold him by your side.\n";
		std::cout << "You tell him everything is going to be alright but we have to go on a trip right now.\n";
		interaction = 1;
	}
}
...
std::string BedRoom3::removeItem(int index) {
